...
def read_multiomics_dataset(data_dir: str | Path) -> tuple[List[np.ndarray], np.ndarray, List[List[str]]]:
    ...
    while True:
        ...
        if feat_path.exists():
            ...
            names = feat_df.iloc[:, 0].astype(str).tolist()
            if len(names) != df.shape[1]:
                raise ValueError(
                    f"Feature-name dimension mismatch for {feat_path}: "
                    f"found {len(names)} names for {df.shape[1]} columns."
                )
            feature_names.append(names)
        else:
            feature_names.append(
                [f"omics{omics_idx}_f{j}" for j in range(df.shape[1])]
            )
        omics_idx += 1
...
def train_final_and_evaluate(
    ...
    y_test_true, y_test_prob = predict_proba(
    ...
    plot_roc(
        ...
        y_test_prob,
        num_classes=num_classes,
        save_path=repeat_dir / "test_roc.png",
    )

    predictions = pd.DataFrame(
        {
            "sample_index": np.asarray(test_indices, dtype=int),
            "y_true": y_test_true.astype(int),
            "y_pred": np.argmax(y_test_prob, axis=1).astype(int),
        }
    )
    for class_index in range(num_classes):
        predictions[f"prob_class_{class_index}"] = y_test_prob[:, class_index]
    predictions.to_csv(repeat_dir / "predictions_test.csv", index=False)
    ...
    metrics_for_json.update(
        {
            ...
            "model": train_cfg.model_type,
            "n_train": int(len(y_train)),
            "n_test": int(len(y_test)),
            "num_classes": int(num_classes),
        }
    )
    with (repeat_dir / "metrics_test.json").open(
        "w", encoding="utf-8"
    ) as handle:
        json.dump(
            metrics_for_json,
...
def run_repeated_holdout_experiment(
    train_cfg: TrainingConfig,
    device: torch.device = DEFAULT_DEVICE,
) -> Dict[str, Any]:
    seed_everything(train_cfg.random_seed)
    output_dir = ensure_dir(train_cfg.output_dir)
    omics_list, labels, feature_names = read_multiomics_dataset(
        train_cfg.data_dir
    )
    ...
    dataset_manifest = {
        ...
        "class_counts": {
            ...
            for class_index, count in enumerate(
                np.bincount(labels, minlength=len(np.unique(labels)))
            )
        },
        "raw_input_dims": [int(array.shape[1]) for array in omics_list],
        "feature_name_files_expected": [
            f"{index}_featname.csv"
            for index in range(1, len(omics_list) + 1)
        ],
    }
    ...
    if train_cfg.split_file is None:
        splitter = StratifiedShuffleSplit(
            n_splits=train_cfg.repeats,
            test_size=train_cfg.test_size,
            random_state=train_cfg.random_seed,
        )
        split_pairs = list(splitter.split(np.zeros_like(labels), labels))
    else:
        split_path = Path(train_cfg.split_file)
        if not split_path.is_file():
            raise FileNotFoundError(split_path)
        split_table = pd.read_csv(split_path)
        required_columns = {"repeat", "sample_index", "subset"}
        missing_columns = required_columns.difference(split_table.columns)
        if missing_columns:
            raise ValueError(
                f"Fixed split file is missing columns: {sorted(missing_columns)}"
            )
        split_pairs = []
        for repeat_index in range(1, train_cfg.repeats + 1):
            current = split_table[split_table["repeat"].eq(repeat_index)]
            train_indices = current.loc[
                current["subset"].eq("train"), "sample_index"
            ].astype(int).to_numpy()
            test_indices = current.loc[
                current["subset"].eq("test"), "sample_index"
            ].astype(int).to_numpy()
            if len(train_indices) == 0 or len(test_indices) == 0:
                raise ValueError(
                    f"Repeat {repeat_index} lacks a train or test subset."
                )
            if np.intersect1d(train_indices, test_indices).size:
                raise ValueError(
                    f"Repeat {repeat_index} has overlapping train/test indices."
                )
            combined_indices = np.concatenate([train_indices, test_indices])
            if combined_indices.min() < 0 or combined_indices.max() >= len(labels):
                raise IndexError(
                    f"Repeat {repeat_index} contains out-of-range sample indices."
                )
            if len(np.unique(combined_indices)) != len(labels):
                raise ValueError(
                    f"Repeat {repeat_index} does not partition all {len(labels)} samples exactly once."
                )
            if "encoded_label" in current.columns:
                expected_labels = current["sample_index"].astype(int).map(
                    lambda index: int(labels[index])
                ).to_numpy()
                supplied_labels = current["encoded_label"].astype(int).to_numpy()
                if not np.array_equal(expected_labels, supplied_labels):
                    raise ValueError(
                        f"Repeat {repeat_index} contains encoded labels inconsistent with labels_all.csv."
                    )
            split_pairs.append((train_indices, test_indices))

    repeat_rows: List[Dict[str, Any]] = []
    split_rows: List[Dict[str, Any]] = []
    for repeat_index, (tr_idx, te_idx) in enumerate(split_pairs, start=1):
        for sample_index in tr_idx:
            split_rows.append(
                {
                    "repeat": repeat_index,
                    "sample_index": int(sample_index),
                    "subset": "train",
                    "encoded_label": int(labels[sample_index]),
                }
            )
        for sample_index in te_idx:
            split_rows.append(
                {
                    "repeat": repeat_index,
                    "sample_index": int(sample_index),
    ...
    split_df.to_csv(output_dir / "outer_splits.csv", index=False)
